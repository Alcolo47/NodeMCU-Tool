...
async function getConnection() {
    ...
    if (_options.device == null) {
        const dn = "/dev/ttyUSB"
        for (let i = 0; i < 10; i++) {
            if (await _fs.exists(dn + i)) {
                _options.device = dn + i;
            }
        }
    }
...
async function upload(localFiles, options, onProgess) {
    ...
    async function uploadFile(localFile, remoteFilenameRelative) {
        ...
        if (options.compile && _path.extname(localFile).toLowerCase() === ".lua" && _path.basename(localFile) !== "init.lua") {
    ...
    if (localFiles.length === 1) {
        ...
        // modified files ?
        if (options.modified) {
            const jsonFile = '.nodemcu-tools.json';

            const filesOldDate = await _fs.exists(jsonFile) ? JSON.parse(await _fs.readFile(jsonFile)) : {};

            const filesDate = {}
            for (const filename of localFiles) {
                filesDate[filename] = (await _fs.stat(filename)).mtimeMs;
            }

            localFiles = localFiles.filter(filename => filesDate[filename] > (filesOldDate[filename] || 0));
            await _fs.writeFile(jsonFile, JSON.stringify(filesDate));
        }
